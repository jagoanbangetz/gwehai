...
const Dashboard = () => {
  ...
  const handleStopJob = async () => {
    ...
    if (eventSourceRef.current) { eventSourceRef.current.close(); eventSourceRef.current = null }
    pentestJobStreamAbortRef.current?.abort(); pentestJobStreamAbortRef.current = null
    try { await gwehaiClient.stopJob(currentJobId) } catch (_e) { showToast('Failed to stop job.', 'error') }
  ...
  return (
    <div className="dashboard">
      ...
      <DashboardLayout
        ...
        sidebarHeader={<SidebarHeader isMobile={isMobile} sidebarCollapsed={sidebarCollapsed} onToggleCollapse={() => setSidebarCollapsed((c) => !c)} onNewChat={handleNewChat} onToggleSearch={() => setShowSearch(!showSearch)} showSearch={showSearch} searchQuery={searchQuery} onSearchQueryChange={setSearchQuery} onShowReport={() => setShowReportModal(true)} onShowHacktivity={() => setShowHacktivityModal(true)} onShowPlan={() => setShowPlanModal(true)} onShowCurrentPentest={() => setShowCurrentPentestModal(true)} />}
