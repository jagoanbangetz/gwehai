...
#
# Alternative: run tools inside Docker — see Dockerfile.pentest-tools and README.md
# ("Running pentest tools in Docker"). Then: docker compose up -d pentest-tools
# and run commands via: docker exec gwehai-pentest-tools <cmd>
...
install_system() {
  if [ "$OS_TYPE" = "deb" ]; then
    ...
    run $PKG_UPDATE
    run $PKG_INSTALL nmap masscan bind whois curl netcat tcptraceroute
